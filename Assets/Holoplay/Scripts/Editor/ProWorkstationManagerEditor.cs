// Pro Workstation is only supported on 2018 or later
...
namespace LookingGlass.ProWorkstation {
	...
	[CustomEditor(typeof(ProWorkstationManager))]
	public class ProWorkstationManagerEditor : Editor {

        const string proBuildDirPrefsName = "_PROBUILDDIR";
        string separator = ProWorkstationManager.separator;
        ...
        public override void OnInspectorGUI() {
            if (Application.platform == RuntimePlatform.OSXEditor) {
                EditorGUILayout.HelpBox(
                    "Pro Workstation is Windows Only. Please only build for windows",
                    MessageType.Error
                );
            }
            ...
            EditorGUILayout.LabelField(
                "Open or create the side panel scene",
            ...
            EditorGUILayout.Space();
            EditorGUILayout.LabelField(
                "Build for the Pro Workstation. " +  
                "This is a special build command that creates two executables that automatically work for the Pro Workstation " +
            ...
            if (GUILayout.Button("Build (Pro Workstation)")) {
            ...
            if (GUILayout.Button("Build and Run (Pro Workstation)")) {
                BuildProScenes(true);
            }
            EditorGUILayout.Space();
            EditorGUILayout.LabelField(
                "Open the folder containing the Pro Workstation build",
                EditorStyles.wordWrappedLabel
            );
        ...
        public void BuildProScenes(bool buildAndRun) {
            ...
            foreach (var s in EditorBuildSettings.scenes) {
                if (s.enabled) {
                    if (s.path.Contains(ProWorkstationManager.extendedUIString)) {
            ...
            if (sceneNamesLKG.Count == 0) {
                ...
                buildPlayerOptions.locationPathName = 
                    ...
                    ProWorkstationManager.lkgDisplayString + ".exe";
                buildPlayerOptions.target = BuildTarget.StandaloneWindows64;
                buildPlayerOptions.options = BuildOptions.None;
                BuildReport report = BuildPipeline.BuildPlayer(buildPlayerOptions);
                BuildSummary summary = report.summary;
